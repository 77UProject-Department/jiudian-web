...
const assets = new Set([config.hero.image, ...config.gallery.map(x => x.image)]);
...
for (const [route, title] of Object.entries(titles)) {
  ...
  await writeFile(path.join(output, route, 'index.html'), html.replace(/<title>.*?<\/title>/, `<title>${title}｜${config.company}</title>`));
}
